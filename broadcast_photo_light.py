#!/usr/bin/env python3
"""
Broadcast photo-as-light to quantum computers
"""
...
import numpy as np
from photo_to_luxbin import main as get_photo_data
...
def broadcast_to_ibm(qc, backend_name='ibm_fez'):
    ...
    try:
        ...
        print(f"Broadcasting photo-as-light to {backend_name}...")
...
def main():
    # Get photo data
    img, binary_data, luxbin_chars, wavelengths = get_photo_data()

    print(f"Encoding {len(wavelengths)} light wavelengths from photo...")
    ...
    for backend in backends:
        ...
        if job_id:
            print(f"Successfully broadcast photo to {backend}")
