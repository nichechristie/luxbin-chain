...
contract DeployLuxbinUSDCStakingLowMin is Script {
    function run() external {
        ...
        // Deploy the low-minimum version
        LuxbinUSDCStaking_LowMin staking = new LuxbinUSDCStaking_LowMin(usdcAddress, oracleAddress);

        vm.stopBroadcast();

        // Log the deployment
        console.log("LuxbinUSDCStaking_LowMin deployed to:", address(staking));
        console.log("USDC Address:", usdcAddress);
        console.log("Oracle Address:", oracleAddress);
        console.log("MINIMUM STAKE: 10 USDC (Low Minimum Version)");
    }
}